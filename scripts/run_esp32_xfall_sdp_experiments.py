...
DEFAULT_DATASET_ROOT = PROJECT_ROOT / "dataset" / "esp32_xfall_sdp_htltf_w100_l20_s10_tol4000"
DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / "artifacts" / "esp32_xfall_sdp_cnn_w100_mps"
TRAIN_SCRIPT = PROJECT_ROOT / "scripts" / "train_esp32_xfall_sdp_cnn_torch.py"
SEEDS = [42, 43, 44]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run 3-seed 2D CNN experiments on esp32 XFall SDP dataset."
    )
    parser.add_argument(
        "--python-bin",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Root directory containing the SDP dataset.",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
        default=DEFAULT_OUTPUT_ROOT,
        help="Directory for run outputs and aggregated summary.",
    )
...
def main() -> None:
    ...
    for seed in SEEDS:
        ...
        per_seed_results.append(json.loads((output_dir / "training_summary.json").read_text()))
