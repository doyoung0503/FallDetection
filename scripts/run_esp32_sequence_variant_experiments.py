...
DEFAULT_DATASET_ROOT = PROJECT_ROOT / "dataset" / "esp32_sequence_htltf_variants_w64_s20"
DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / "artifacts" / "esp32_sequence_variant_cnn_mps"
TRAIN_SCRIPT = PROJECT_ROOT / "scripts" / "train_esp32_sequence_cnn_torch.py"

VARIANTS = ["interp_only", "interp_mask", "interp_mask_deltat"]
SEEDS = [42, 43, 44]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run 3-seed sequence CNN experiments on esp32 HT-LTF sequence variants."
    )
    parser.add_argument(
        "--python-bin",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Root containing variant directories.",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
...
def main() -> None:
    ...
    for variant in VARIANTS:
        variant_root = args.dataset_root / variant / f"windows_{args.window_length}"
        ...
        for seed in SEEDS:
            output_dir = variant_output_root / f"seed_{seed}"
            command = [
                str(args.python_bin),
                str(TRAIN_SCRIPT),
                "--variant-root",
